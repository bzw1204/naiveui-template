...
<template>
  <!-- <n-el
    class="m-x-2px cursor-pointer b b-[--divider-color] b-b-[#0000] rounded-[--border-radius] p-x-4 p-y-2"
    :class="[
  ...
  >
    <div class="flex-center gap-2 text-nowrap">
      ...
      <button
        v-if=""
        type="button"
        class="h-18px w-18px flex-center rounded-3px bg-transparent text-[var(--close-icon-color)] hover:bg-[var(--close-color-hover)]"
  ...
  <n-tag
    :type="route.fullPath === value ? 'success' : 'default'"
    :closable="closable"
    :bordered="false"
    class="m-x-2px cursor-pointer"
    @close="() => emit('close', route.fullPath)"
  >
    {{ route.meta.title }}
  </n-tag>
</template>
